...
avi::DefectRes avi::displacedAtoms2defects(
  ...
  */
  ...
  std::vector<std::pair<avi::Coords, int>> vacIn_(vacIn.size());
  for (int i =0 ; i < vacIn.size(); i++) {
	  vacIn_[i] = std::make_pair(vacIn[i], i);
  }
  //std::vector<double> distVac2(siaIn.size());
  //std::vector<int> vacOcc_(siaIn.size());
  std::vector<std::pair<int, double>> vacOccDist(siaIn.size());

  std::transform(std::execution::seq, std::begin(siaIn), std::end(siaIn), std::begin(vacOccDist), [vacIn_, box] (auto sia) {
      double minDistance = std::numeric_limits<double>::max();
      int minIndex = -1;
      std::pair<int, double> init = std::make_pair(minIndex, minDistance);
 //std::make_pair(minIndex, minDistance);
      auto redRes = std::reduce(std::execution::par, vacIn_.begin(), vacIn_.end(), init, [sia](std::pair<int, double> x, std::pair<avi::Coords, int> y) {
                        std::pair<int, double> init = std::make_pair(0, 0.1);
                        double distance = (sia.first[0]-y.first[0])*(sia.first[0]-y.first[0]) + (sia.first[1]-y.first[1])*(sia.first[1]-y.first[1]);//avi::calcDistSqr(sia.first, vacIn[i], box);

                        if (distance < x.second) {
			  return std::make_pair(y.second, distance);
      ...
      return std::make_pair(minIndex, minDistance); //distVac2[i] = minDistance; vacOcc_[i]  = minIndex;
      */
  });

  std::vector<double> distVac2;//(siaIn.size());
  std::vector<int> vacOcc_;//(siaIn.size());
  distVac2.reserve(siaIn.size());
  vacOcc_.reserve(siaIn.size());
  for (const auto &it : vacOccDist) {
	  vacOcc_.push_back(it.first);
	  distVac2.push_back(it.second);
  }

  /*

  std::for_each(std::execution::par, std::begin(siaIn), std::end(siaIn), [&vacIn, &distVac2, &vacOcc_, box] (const auto& sia) {
      double minDistance = std::numeric_limits<double>::max();
      int minIndex = -1;
      for (int i = 0; i < vacIn.size(); ++i) {
          double distance = avi::calcDistSqr(sia.first, vacIn[i], box);
  ...
  */
  ...
  if (config.isAddThresholdDefects) {
    ...
    //std::vector<int> siaThreshExcl, vacThreshExcl;
    for (int i = 0; i < siaThreshExcl.size(); i++) {
