...
auto cleanDisplaced(avi::DefectVecT &defect, double thresh_, double box) {
  ...
  std::for_each(defect.begin(), defect.end(), [&defect, thresh, box](auto &def) {
    if (!isInterstitial(def) || isSurviving(def)) {
      ...
      for (size_t j = 0; j < defect.size(); ++j) {
        ...
        auto dist = avi::calcDistSqr(coords(def), coords(defect[j]), box);
        if (dist < min) {
          min = dist;
          //minj = j;
        }
...
avi::DefectRes avi::displacedAtoms2defects(
  ...
  */
  ...
  auto ptrDistVac2 = distVac2.data();
  auto ptrVacOcc_ = vacOcc_.data();
  auto ptrVacIn = vacIn.data();
  auto sz = vacIn.size();

  std::for_each(std::execution::par, std::begin(siaIn), std::end(siaIn),
    [box, sz, ptrVacIn, ptrDistVac2, ptrVacOcc_] (auto sia) {
      double minDistance = std::numeric_limits<double>::max();
      int minIndex = -1;
      for (int i = 0; i < sz; ++i) {
          double distance = avi::calcDistSqr(sia.first, ptrVacIn[i], box);
          if (distance < minDistance) {
              minDistance = distance;
              minIndex = i;
          }
      }
      ptrDistVac2[sia.second] = minDistance;//.push_back(minDistance);
      ptrVacOcc_[sia.second] = minIndex;//.push_back(minIndex);
    });
  /*
  ...
  */
  ...
  std::sort(std::execution::seq, begin(vacRecomb), end(vacRecomb));
  ...
  std::sort(std::execution::seq, sortOrder.begin(), sortOrder.end(),
              [&](int i1, int i2) { return vacRecombInverted[i1] < vacRecombInverted[i2]; });

  std::vector<int> sia(indexOfVacRecomb.size());
  std::transform(std::execution::seq, sortOrder.begin(), sortOrder.end(), sia.begin(),
                   [&](int i) { return indexOfVacRecomb[i]; });

  std::vector<int> vacRecombNonUnique(vacRecombInverted.size());
  std::transform(std::execution::seq, sortOrder.begin(), sortOrder.end(), vacRecombNonUnique.begin(),
