...
struct PointCloudAdaptor4Clean {
  ...
  explicit PointCloudAdaptor4Clean(const avi::DefectVecT& points, double boxSize) : points{points}, boxSize{boxSize} {}
  ...
  inline double kdtree_distance(const double* p1, const size_t idx_p, const size_t /*idx_q*/, size_t /*size*/) const {
    ...
    const double dx_periodic = fmin(fabs(dx), fabs(dx - boxSize));
    const double dy_periodic = fmin(fabs(dy), fabs(dy - boxSize));
    const double dz_periodic = fmin(fabs(dz), fabs(dz - boxSize));
...
auto cleanDisplaced(avi::DefectVecT &defects, double thresh_, double box) {
  using avi::DefectTWrap::coords;
  ...
  params.sorted = true;
  std::for_each(std::execution::seq, begin(defects), end(defects), [&defects, &kdTree, thresh, &matches, &params](auto &defect){
    matches.clear();
...
struct PointCloudAdaptor {
  ...
  explicit PointCloudAdaptor(const std::vector<avi::Coords>& points, double boxSize) : points{points}, boxSize{boxSize} {}
...
avi::DefectRes avi::displacedAtoms2defects(
  ...
  */
  ...
  std::for_each(std::execution::seq, begin(siaIn), end(siaIn), [&kdTree, &neighborDistances, &neighborIndices, numNeighbors, &distVac2, &vacOcc_](const auto &sia) {
